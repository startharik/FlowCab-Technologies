    ...
    @IBAction func onWithdrawClicked(_ sender: UIButton) {
        ...
        dialog.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: "Message OK button"), style: .default) { action in
            RequestPayment().execute() { result in
                switch result {
                case .success(_):
                    let popup = DialogBuilder.getDialogForMessage(message: NSLocalizedString("Withdraw request was successfully applied. Your payment will be done soon.", comment: ""), completion: nil)
