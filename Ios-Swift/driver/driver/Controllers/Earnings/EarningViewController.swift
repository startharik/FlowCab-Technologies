...
class EarningViewController: UIViewController {
    ...
    func refresh(queryType: QueryType) {
        GetStats(query: queryType).execute() { result in
            ...
            case .success(let response):
                ...
                self.textDistance.text = MKDistanceFormatter().string(fromDistance: Double(current.distance)!)
    ...
    func setEarningChart(dataPoints: [DataPoint]) {
        ...
        for i in 0..<dataPoints.count {
            let dataEntry = BarChartDataEntry(x: Double(i), yValues: [dataPoints[i].earning])
        ...
        chartDataSet.colors = [self.view.tintColor]
