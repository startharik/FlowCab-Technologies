...
class EditProfileViewController: FormViewController {
    ...
    @IBAction func onSaveButtonClicked(_ sender: UIBarButtonItem) {
        Register(jwtToken: UserDefaultsConfig.jwtToken!, driver: driver).execute() { result in
            switch result {
            case .success(_):
                let dialog = DialogBuilder.getDialogForMessage(message: NSLocalizedString("Registration was done successfully. You can exit the app now and check back later to see your approval status.", comment: "Registration successful")) { result in
