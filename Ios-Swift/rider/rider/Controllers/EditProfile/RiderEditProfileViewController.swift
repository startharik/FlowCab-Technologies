    ...
    override func viewDidLoad() {
        ...
        form +++ Section(NSLocalizedString("Images", comment: "Profile's image section header"))
                    ...
                    if(!self.downloading) {
                        ...
                        UpdateProfileImage(data: data!).execute() { result in
                            ...
                            case .success(let response):
                                DialogBuilder.alertOnSuccess(message: NSLocalizedString("Uploading Profile Image was successful.", comment: "Uploading profile image successful alert message"))
            ...
            <<< PushRow<String>() {
                ...
                }.onChange { self.rider.gender = $0.value! as String }
            <<< TextRow(){
