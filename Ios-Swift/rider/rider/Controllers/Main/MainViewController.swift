...
class MainViewController: UIViewController, CLLocationManagerDelegate, ServiceRequested {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        guard let position = manager.location else {
            let alert = UIAlertController(title: "Message", message: "Couldn't get current location. use search to find your current place on map.", preferredStyle: .alert)
    ...
    @IBAction func onFavoritesClicked(_ sender: UIBarButtonItem) {
        GetAddresses().execute() { result in
            ...
            case .success(let response):
                if(response.count < 1) {
                    let alert = UIAlertController(title: "Message", message: "No Favorite Address found.", preferredStyle: .alert)
