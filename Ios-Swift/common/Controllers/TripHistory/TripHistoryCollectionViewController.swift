...
class TripHistoryCollectionViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    override func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        return UIContextMenuConfiguration(identifier: nil, previewProvider: nil, actionProvider: { suggestedActions in
            let complaint = UIAction(title: "Write Complaint", image: UIImage(systemName: "square.and.pencil")) { action in
                let title = "Complaint"
                let message = "You can write a complaint on service provided and it will be reviewed As soon as possible."
                ...
                dialog.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: "Message OK button"), style: .default) { action in
                    WriteComplaint(requestId: self.travels[indexPath.row].id!, subject: dialog.textFields![0].text!, content: dialog.textFields![1].text!).execute() { result in
                        switch result {
                        case .success(_):
                            DialogBuilder.alertOnSuccess(message: "Complaint Sent.")
            ...
            let hide = UIAction(title: "Hide Item", image: UIImage(systemName: "eye.slash"), attributes: .destructive) { action in
